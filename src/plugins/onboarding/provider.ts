...
export const onboardingProvider: Provider = {
  get: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<string | null> => {
    ...
    // CRITICAL: Check onboarding step FIRST, before any async operations
    // If in onboarding, throw error immediately to prevent LLM generation
    ...
    // CRITICAL: During onboarding (except CONFIRMATION/NONE), throw error to prevent LLM generation
    // Do this BEFORE any other processing to minimize work
    if (step !== 'CONFIRMATION' && step !== 'NONE' && step !== 'COMPLETED') {
...
After sending this message, wait for the user's response.]`;
    ...
    return null;
      ...
      // If this is our onboarding error, re-throw it to prevent LLM generation
      if (error.message === 'ONBOARDING_IN_PROGRESS: Action handler will send message, LLM should not respond') {
        console.log('[Onboarding Provider] Re-throwing onboarding error to prevent LLM generation');
        throw error;
      }
