...
export async function sendFeatureRequest(
  ...
  if (!smtpUser || !smtpPass) {
    console.error('[Feature Request] ❌ SMTP credentials not configured');
  ...
  console.log('[Feature Request] 📧 Attempting to send email...');
...
export async function sendNoMatchNotification(
  ...
  if (!smtpUser || !smtpPass) {
    console.error('[No Match Notification] ❌ SMTP credentials not configured');
    throw new Error('SMTP credentials not configured');
  }

  console.log('[No Match Notification] 📧 Attempting to send no-match notification...');
  ...
  if (runtime) {
    try {
      ...
      if (isMongo && db && db.getDb) {
        ...
        console.log('[No Match Notification] ✅ Tracked manual connection request in MongoDB');
      } else if (db && db.query) {
        // PostgreSQL
      ...
      console.error('[No Match Notification] ⚠️ Could not track in database:', trackErr.message);
...
This email was automatically generated by Agent Kaia when no matches were found.`,
  ...
  try {
    const transporter = createTransporter();
    await transporter.sendMail(mailOptions);
    console.log(`[No Match Notification] ✅ Successfully sent no-match notification to ${NO_MATCH_EMAIL}`);
  } catch (error: any) {
    console.error('[No Match Notification] ❌ Error sending email:', error);
